...
class MeiziSpider(scrapy.Spider):
    ...
    def parse(self, response):
        ...
        for img in lst:
            ...
            base_path = os.path.join("/Users/zhouxiaohang/PycharmProjects/ss", imgname)
            if not os.path.exists(base_path):
                os.makedirs(base_path)

            if imgurl is not None:
                yield scrapy.Request(imgurl, meta={'base_path': base_path}, headers=header, callback=self.content, dont_filter=True)
    ...
    def content(self,response):
        ...
        if next_url is not None:
            yield response.follow(next_url, meta={'base_path': base_path}, headers=header, callback=self.content, dont_filter=True)
